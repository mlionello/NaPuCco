...
**OUTPUT**
Clusterbased and voxelwise errors are repeated K times for each experiments, the results (pass or fail) are then averaged to calculate the power and fpr.

[Project FLOWChART](flowchartNPC.pdf)
...
Experiments results include Power and Fpr analysis for both Voxelwise and Cluster based correction. The result of each experiment is saved in 
  ...
  - **_subj_subsampling(..., 'fpr', 0, 'nullfold', NONEFFFOLD)_** it computes only the power. In the case that the non effected volumes are computed separatedly from the effected ones (see example nr. [[[]]]), **NONEFFFOLD** is needed where taking the non effected samples from.
  **_subj_subsampling(..., 'power', 0)_** it computes only the fpr. *homepath* is assumed to contain non-effected volumes
 ...
 - **_subj_subsampling(...,  'vx_rangeII', [500:500:10000)_** The experimetns will be repeated while varying the size of non-effected volumes (this is a mandatory step to identify the number of non-effected voxels needed for statistics to converge). In the clusterbased correction case, the size of non effected volumes will be determined by the size of effected volumes and calcualted in _utils/resizecubicpadding.m_

 - **TOFIX** **_subj_subsampling(...,  'vx_range',  [3:1:10)_** (Power estimation for lusterbased case) The experimetns will be repeated while varying the size of the effected volumes.
  ...
  - **_compute_mean_res(path)_** it generates the average results for all the experiments simulated by **subj_subsampling**. path must in the format 'data/inXX_tXX/results/XXXXX/grouping'.
